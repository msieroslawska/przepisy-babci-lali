...
const validate = (recipeData: RecipeData): boolean => !!recipeData.imageName && !!recipeData.ingredientsList && !!recipeData.instructions && !!recipeData.name;



const RecipeLayout: React.FC<Props> = (props) => {
  const renderContent = () => validate(props.pageContext) ? <Recipe {...props} /> : renderError();
  return (
